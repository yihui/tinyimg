...
lossy_results = do.call(rbind, lapply(seq_along(plot_files), function(i) {
  input_file = plot_files[i]
  plot_name = plot_names[i]
  original_size = file.info(input_file)$size
  do.call(rbind, lapply(lossy_levels, function(lossy) {
    output_file = file.path(
      tmp_dir, sprintf("%s_lossy_%s.png", plot_name, gsub("\\.", "_", lossy))
    )
    time_taken = system.time({
      tinypng(input_file, output = output_file, level = 6, lossy = lossy)
    })["elapsed"]
