...
```{r lossy-benchmark}
lossy_levels = c(0, 0.25, 0.5, 0.75, 1, NA)
...
for (i in seq_along(plot_files)) {
  ...
  for (lossy in lossy_levels) {
    lossy_label = if (is.na(lossy)) "NA" else as.character(lossy)
...
get_auto_value = function(plot_name, column) {
  x = subset(lossy_results, Plot == plot_name & is.na(Lossy))[[column]]
...
```{r plot-filesize-lossy, fig.cap='File size vs lossy level (with dashed line for lossy = NA).'}
par(mar = c(4, 4, 1, 1))
plot(NA, xlim = c(0, 1), ylim = range(lossy_results$Size_KB), log = "y",
     xlab = "Lossy Level", ylab = "File Size (KB, log scale)")
for (i in seq_along(plot_names)) {
  plot_data = subset(lossy_results, Plot == plot_names[i] & !is.na(Lossy))
...
```{r plot-time-lossy, fig.cap='Processing time vs lossy level (with dashed line for lossy = NA).'}
par(mar = c(4, 4, 1, 1))
...
```{r plot-reduction-pct-lossy, fig.cap='Size reduction percentage by lossy level (with dashed line for lossy = NA).'}
par(mar = c(4, 4, 1, 1))
...
```{r plot-size-reduction-bar-lossy, fig.cap='Absolute size reduction by lossy level (including lossy = NA).'}
par(mar = c(4, 4, 1, 1))
lossy_categories = lossy_levels
lossy_labels = ifelse(is.na(lossy_categories), "NA", as.character(lossy_categories))
lossy_reduction_matrix = do.call(cbind, lapply(plot_names, function(pn) {
  ...
  sapply(lossy_categories, function(k) {
    size_k = subset(plot_data, if (is.na(k)) is.na(Lossy) else !is.na(Lossy) & Lossy == k)$Size_KB
