...
lossy_inputs = c(as.list(lossy_levels), list(TRUE))
...
for (i in seq_along(plot_files)) {
  ...
  for (lossy in lossy_inputs) {
    lossy_label = if (isTRUE(lossy)) "TRUE" else as.character(lossy)
    ...
    lossy_rows[[length(lossy_rows) + 1L]] = data.frame(
      Plot = plot_name,
      Lossy = lossy_label,
      LossyNum = if (isTRUE(lossy)) NA_real_ else as.numeric(lossy),
      Size_KB = new_size / 1024,
      Reduction_pct = ((original_size - new_size) / original_size) * 100,
      Time_sec = time_taken
    )
