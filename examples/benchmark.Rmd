...
lossy_inputs = c(
  lapply(lossy_levels, function(x) list(label = as.character(x), value = x)),
  list(list(label = "TRUE", value = TRUE))
)
...
for (i in seq_along(plot_files)) {
  ...
  for (lossy in lossy_inputs) {
    lossy_label = lossy$label
    lossy_value = lossy$value
    output_file = file.path(
      dirname(input_file),
      sprintf("%s_lossy_%s.png", plot_name, gsub("\\.", "_", tolower(lossy_label)))
    )
    time_taken = system.time({
      tinypng(input_file, output = output_file, lossy = lossy_value)
    })["elapsed"]
    ...
    lossy_rows[[length(lossy_rows) + 1L]] = data.frame(
      Plot = plot_name,
      Lossy = lossy_label,
      LossyNum = if (isTRUE(lossy_value)) NA_real_ else as.numeric(lossy_value),
      Size_KB = new_size / 1024,
      Reduction_pct = ((original_size - new_size) / original_size) * 100,
      Time_sec = time_taken
    )
...
```{r lossy-helpers, include=FALSE}
get_auto_value = function(plot_name, column) {
  x = subset(lossy_results, Plot == plot_name & Lossy == "TRUE")[[column]]
...
```{r plot-filesize-lossy, fig.cap='File size vs lossy level (with dashed line for lossy = TRUE).'}
par(mar = c(4, 4, 1, 1))
plot(NA, xlim = c(0, 1), ylim = range(lossy_results$Size_KB), log = "y",
     xlab = "Lossy Level", ylab = "File Size (KB, log scale)")
for (i in seq_along(plot_names)) {
  plot_data = subset(lossy_results, Plot == plot_names[i] & !is.na(LossyNum))
  lines(plot_data$LossyNum, plot_data$Size_KB,
        type = "b", pch = pchs[i], col = i + 1)
  auto_size = get_auto_value(plot_names[i], "Size_KB")
  if (is.finite(auto_size)) abline(h = auto_size, col = i + 1, lty = 2)
}
legend("topright", legend = plot_names,
...
```{r plot-time-lossy, fig.cap='Processing time vs lossy level (with dashed line for lossy = TRUE).'}
par(mar = c(4, 4, 1, 1))
plot(NA, xlim = c(0, 1), ylim = range(lossy_results$Time_sec),
     xlab = "Lossy Level", ylab = "Time (seconds)")
for (i in seq_along(plot_names)) {
  plot_data = subset(lossy_results, Plot == plot_names[i] & !is.na(LossyNum))
  lines(plot_data$LossyNum, plot_data$Time_sec,
        type = "b", pch = pchs[i], col = i + 1)
  auto_time = get_auto_value(plot_names[i], "Time_sec")
  if (is.finite(auto_time)) abline(h = auto_time, col = i + 1, lty = 2)
}
legend("topleft", legend = plot_names,
       col = seq_along(plot_names) + 1, pch = pchs[seq_along(plot_names)], lty = 1)
grid()
...
```{r plot-reduction-pct-lossy, fig.cap='Size reduction percentage by lossy level (with dashed line for lossy = TRUE).'}
par(mar = c(4, 4, 1, 1))
plot(NA, xlim = c(0, 1), ylim = range(lossy_results$Reduction_pct),
     xlab = "Lossy Level", ylab = "Size Reduction (%)")
for (i in seq_along(plot_names)) {
  plot_data = subset(lossy_results, Plot == plot_names[i] & !is.na(LossyNum))
  lines(plot_data$LossyNum, plot_data$Reduction_pct,
        type = "b", pch = pchs[i], col = i + 1)
  auto_reduction = get_auto_value(plot_names[i], "Reduction_pct")
  if (is.finite(auto_reduction)) abline(h = auto_reduction, col = i + 1, lty = 2)
}
legend("bottomright", legend = plot_names,
       col = seq_along(plot_names) + 1, pch = pchs[seq_along(plot_names)], lty = 1)
grid()
...
```{r plot-size-reduction-bar-lossy, fig.cap='Absolute size reduction by lossy level (including lossy = TRUE).'}
par(mar = c(4, 4, 1, 1))
lossy_categories = c(as.character(lossy_levels), "TRUE")
lossy_reduction_matrix = do.call(cbind, lapply(plot_names, function(pn) {
  ...
  sapply(lossy_categories, function(k) {
    raw_size - subset(plot_data, Lossy == k)$Size_KB
  })
}))
barplot(t(lossy_reduction_matrix),
        beside = TRUE,
        names.arg = lossy_categories,
        col = seq_along(plot_names) + 1,
        xlab = "Lossy Level",
        ylab = "Size Reduction (KB)",
        legend.text = plot_names,
...
files = unlist(lossy_files, use.names = FALSE)
if (length(files)) unlink(files[file.exists(files)], force = TRUE)
```
