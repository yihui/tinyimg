...
When building from a git checkout, you need to vendor the Rust dependencies first:
...
1. Run `./update-vendor.sh` to create `vendor.tar.xz`
2. Build the R package with `R CMD build` (includes `vendor.tar.xz`)
3. The CRAN tarball contains `vendor.tar.xz`, not the uncompressed `vendor/` directory
4. During installation, Makevars extracts `vendor.tar.xz` if present
